...
namespace fdeep
{
	...
	private:
		...
		friend model read_model( QFile&, bool,
		                         const std::function<void( std::string )>&, float_type,
		                         const internal::layer_creators& );
...
// Load and construct an fdeep::model from an QFile
// providing the exported json content.
// Throws an exception if a problem occurs.
	inline model read_model( QFile& model_file_stream,
	                         bool verify = true,
	                         const std::function<void( std::string )>& logger = cout_logger,
	                         float_type verify_epsilon = static_cast<float_type>(0.0001),
	                         const internal::layer_creators& custom_layer_creators = internal::layer_creators())
	{
		...
		log_sol( "Loading json" );
		nlohmann::json json_data = nlohmann::json::parse( model_file_stream.readAll().data());
		log_duration();
...
// Throws an exception if a problem occurs.
	...
	/*  QFile overload */
	inline model load_model( QFile& file,
	                         bool verify = true,
	                         const std::function<void( std::string )>& logger = cout_logger,
	                         float_type verify_epsilon = static_cast<float_type>(0.0001),
	                         const internal::layer_creators& custom_layer_creators = internal::layer_creators())
	{
		...
		internal::assertion( file.open( QIODevice::ReadOnly | QIODevice::Text ), "Can not open " + file.fileName().toStdString());
		const auto model = read_model( file, verify, logger, verify_epsilon,
		                               custom_layer_creators );
		if( logger ) {
			const std::string additional_action = verify ? ", testing" : "";
			logger( "Loading, constructing" + additional_action +
			        " of " + file.fileName().toStdString() + " took " +
			        fplus::show_float( 0, 6, stopwatch.elapsed()) + " s overall.\n" );
		}
		return model;
	}
