...
namespace dsdk
{
	...
	public:
		/*!
		 * @brief Structure to hold the pos / val pair to generate the solved sudoku
		 */
		struct ReturnVal
		{
			int position;
		...
		/*!
		 * @brief initialized on first startup.
		 * @param path Path to the trained NN Json model.
		 */
		explicit DeepSolver( const std::string& path );
		/*!
		 * @brief initialized on first startup.
		 * @note Required to access Androids filesystem
		 * @param path QFile with the path to the traines NN Json model.
		 */
