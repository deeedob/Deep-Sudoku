...
namespace dsdk
{
	...
	{
		/*!
		 * @brief Parameters used to detect numbers in a cutted square
		 */
	...
	public:
		/*!
		 * @brief Constructs the class with an QImage
		 * @note The image gets internally converted to cv::Mat using qt_ocv::image2Mat
		 * @param src the image to process
		 */
		explicit CVSegmentation( const QImage& src );
		explicit CVSegmentation( const CVSegmentation& other ) = delete;
		...
		/*!
		 * @brief Main processing function that handles the whole call stack
		 * @return true if processing was fully successfully false otherwise
		 */
		bool process();
		...
		/*!
		 * @brief Contains only the fields of the grid that contained a identified number
		 * @return returns a vector of the identified images
		 * @note only images are returned. To resolve the correct field use getUsedSquareNums()
		 */
		[[nodiscard]] const std::vector<cv::Mat>& getPreparedSquares() const noexcept;
		/*!
		 * @brief a vector of ints that has the same size as the images returned from getPreparedSquares()
		 * @return the position that correlate with the identified numbers.
		 */
